...
import {
  SOLAR_COLLECTION_IDS,
  SOLAR_COLLECTION_HANDLES,
  SOLAR_CATEGORY_IDS,
  SOLAR_CATEGORY_HANDLES,
...
export const metadata: Metadata = {
  ...
  description:
    "Solar panels, inverters, batteries and power stations for homes and businesses. Reliable power solutions at great prices on CeedMart.",
}
...
const solarProductFilter = {
  collection_id: SOLAR_COLLECTION_IDS,
}

export default async function SolarEnergyPowerStorePage(props: Params) {
  ...
  const [categories, collectionsData, productsData, region, cart] =
    await Promise.all([
      ...
      listProducts({
        pageParam: 1,
        countryCode,
        queryParams: { limit: 12, ...solarProductFilter },
      }),
      getRegion(countryCode),
      retrieveCart().catch(() => null),
    ])

  const solarCategories = (categories || [])
    .filter(
      (c) =>
        !c.parent_category &&
        (SOLAR_CATEGORY_IDS.includes(c.id) ||
          SOLAR_CATEGORY_HANDLES.includes(c.handle ?? ""))
    )
    .map((c) => ({
      id: c.id,
      name: c.name,
  ...
  const solarCollections = (collectionsData?.collections || []).filter(
    (c) =>
      SOLAR_COLLECTION_IDS.includes(c.id) ||
      SOLAR_COLLECTION_HANDLES.includes(c.handle ?? "")
  )
  ...
  return (
    <div className="flex flex-col gap-0 bg-tech-bg min-h-screen">
        ...
        <div className="content-container relative py-10 small:py-14">
          <div className="flex items-center gap-3 mb-2">
            <svg viewBox="0 0 32 32" fill="none" className="w-8 h-8">
              <circle cx="16" cy="10" r="4" fill="#FFCE00" fillOpacity="0.9" />
              <path
                d="M16 3v3m0 8v3M8 10h3m10 0h3M9 5l2 2m10 10l2 2M9 15l2-2m10-10l2 2"
                stroke="white"
                strokeWidth="1.5"
                strokeLinecap="round"
              />
              <rect x="5" y="18" width="22" height="10" rx="1" stroke="white" strokeWidth="1.5" />
              <line x1="9" y1="18" x2="9" y2="28" stroke="white" strokeWidth="1" />
              <line x1="16" y1="18" x2="16" y2="28" stroke="white" strokeWidth="1" />
              <line x1="23" y1="18" x2="23" y2="28" stroke="white" strokeWidth="1" />
            </svg>
          ...
          <p className="text-white/80 text-base small:text-lg mt-3 max-w-lg">
            Solar panels, inverters, batteries and power stations for homes
            and businesses. Reliable power at great prices.
          </p>
      ...
      <div className="content-container py-6 flex flex-col gap-4">
        <CollectionsRichGrid collections={solarCollections} limit={6} />
        ...
        <section className="mt-8">
          ...
          <InfiniteProductGrid
            ...
            countryCode={countryCode}
            region={region}
            queryParams={solarProductFilter}
