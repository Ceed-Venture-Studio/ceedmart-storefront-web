...
function buildWhatsAppMessage(cart: HttpTypes.StoreCart) {
  const items = cart.items
    ?.map(
      (item) =>
        `- ${item.product_title}${item.variant?.title && item.variant.title !== "Default" ? ` (${item.variant.title})` : ""} x${item.quantity}`
    )
    .join("\n")
  ...
  return encodeURIComponent(
    `Hello CeedMart,\n\nI would like to place a wholesale/bulk pre-order for the following items:\n\n${items}\n\nTotal: ${total}\nCart ID: ${cart.id}\n\nPlease let me know how to proceed with payment and delivery. Thank you!`
  )
}
  ...
  return (
    <div className="flex flex-col gap-y-4">
      ...
      <div className="flex flex-col gap-y-3">
        ...
        >
          <Button className="w-full h-10 bg-ceedmart-navy hover:bg-ceedmart-navy-light">
            Pay Now
          </Button>
        </LocalizedClientLink>

        {/* Pre-Order via WhatsApp — pay later */}
        <a
